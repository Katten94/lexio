...
export default function SignupPage() {
  ...
  const opret = async () => {
    ...
    if (error) { setFejl(error.message); setLoader(false) }
    ...
    <div className="min-h-screen flex" style={{ backgroundColor: '#F9FAFB' }}>
      ...
      <div className="hidden lg:flex flex-col w-2/5 px-12"
        ...
        <Image
          ...
          width={160}
          height={48}
          style={{ width: '160px', height: 'auto', marginBottom: '80px' }}
          priority
        />

        <div>
          <p style={{ fontFamily: 'Arial', fontSize: '11px', color: 'rgba(255,255,255,0.4)', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '12px' }}>
            Opret din konto
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <Trin nr={1} label="Dine oplysninger" aktiv={trin === 1} faerdig={trin > 1} />
            <div style={{ width: '1px', height: '10px', backgroundColor: 'rgba(255,255,255,0.15)', marginLeft: '10px' }} />
        ...
        <div style={{ width: '360px' }}>
          ...
          {trin === 1 && (
            <div>
              ...
              <div className="mb-4">
                ...
                <input type="text" value={navn} onChange={e => setNavn(e.target.value)} placeholder="Dit navn" className="w-full px-4 py-2.5 rounded-lg outline-none" style={{ border: '1px solid #E5E7EB', fontSize: '13px', fontFamily: 'Arial', color: '#111827', backgroundColor: 'white' }} />
              </div>
              <div className="mb-4">
                <label className="block mb-1.5" style={{ fontSize: '12px', fontWeight: '600', color: '#374151', fontFamily: 'Arial' }}>Email</label>
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="din@skole.dk" className="w-full px-4 py-2.5 rounded-lg outline-none" style={{ border: '1px solid #E5E7EB', fontSize: '13px', fontFamily: 'Arial', color: '#111827', backgroundColor: 'white' }} />
              </div>
              <div className="mb-8">
                <label className="block mb-1.5" style={{ fontSize: '12px', fontWeight: '600', color: '#374151', fontFamily: 'Arial' }}>Adgangskode</label>
                <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Mindst 6 tegn" className="w-full px-4 py-2.5 rounded-lg outline-none" style={{ border: '1px solid #E5E7EB', fontSize: '13px', fontFamily: 'Arial', color: '#111827', backgroundColor: 'white' }} />
              </div>
              <button onClick={() => { if (navn && email && password.length >= 6) setTrin(2) }}
                ...
                style={{ backgroundColor: navn && email && password.length >= 6 ? '#0F2A5E' : '#E5E7EB', color: navn && email && password.length >= 6 ? 'white' : '#9CA3AF', fontSize: '14px', fontFamily: 'Arial', cursor: navn && email && password.length >= 6 ? 'pointer' : 'not-allowed' }}>
                Fortsæt →
              </button>
              <p className="text-center" style={{ fontSize: '13px', color: '#6B7280', fontFamily: 'Arial' }}>
                Har du allerede en konto?{' '}<a href="/login" style={{ color: '#0F2A5E', fontWeight: '600', textDecoration: 'none' }}>Log ind</a>
              </p>
            </div>
          )}
          ...
          {trin === 2 && (
            <div>
              ...
              <p className="mb-6" style={{ fontSize: '13px', color: '#6B7280', fontFamily: 'Arial' }}>Trin 2 af 4 — vælg de fag du underviser i</p>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button onClick={() => setTrin(1)} className="flex-1 py-2.5 rounded-lg font-medium" style={{ backgroundColor: 'white', border: '1px solid #E5E7EB', color: '#374151', fontSize: '14px', fontFamily: 'Arial' }}>← Tilbage</button>
                <button onClick={() => setTrin(3)} className="flex-1 py-2.5 rounded-lg font-medium" style={{ backgroundColor: '#0F2A5E', color: 'white', fontSize: '14px', fontFamily: 'Arial', cursor: 'pointer' }}>Fortsæt →</button>
              </div>
            </div>
          )}
          ...
          {trin === 3 && (
            <div>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button onClick={() => setTrin(2)} className="flex-1 py-2.5 rounded-lg font-medium" style={{ backgroundColor: 'white', border: '1px solid #E5E7EB', color: '#374151', fontSize: '14px', fontFamily: 'Arial' }}>← Tilbage</button>
                <button onClick={() => setTrin(4)} className="flex-1 py-2.5 rounded-lg font-medium" style={{ backgroundColor: '#0F2A5E', color: 'white', fontSize: '14px', fontFamily: 'Arial', cursor: 'pointer' }}>Fortsæt →</button>
              </div>
            </div>
          )}
          ...
          {trin === 4 && (
            <div>
              ...
              <p className="mb-6" style={{ fontSize: '13px', color: '#6B7280', fontFamily: 'Arial' }}>Trin 4 af 4 — tjek dine oplysninger</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '24px' }}>
                {[{ lbl: 'Navn', val: navn }, { lbl: 'Email', val: email }].map(({ lbl, val }) => (
                  <div key={lbl} className="rounded-lg px-4 py-3" style={{ backgroundColor: 'white', border: '1px solid #E5E7EB' }}>
                    <p className="text-xs mb-0.5" style={{ color: '#9CA3AF', fontFamily: 'Arial', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{lbl}</p>
                    <p className="text-sm font-medium" style={{ color: '#111827', fontFamily: 'Arial' }}>{val}</p>
                  </div>
                ))}
                <div className="rounded-lg px-4 py-3" style={{ backgroundColor: 'white', border: '1px solid #E5E7EB' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                    {forloeb.length === 0
                      ? <p className="text-xs" style={{ color: '#9CA3AF' }}>Ingen valgt</p>
                      : forloeb.map(f => (
                        <span key={f} className="text-xs px-2 py-0.5 rounded" style={{ backgroundColor: '#F9FAFB', border: '1px solid #E5E7EB', color: '#374151', fontFamily: 'Arial' }}>{f}</span>
                      ))}
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button onClick={() => setTrin(3)} className="flex-1 py-2.5 rounded-lg font-medium" style={{ backgroundColor: 'white', border: '1px solid #E5E7EB', color: '#374151', fontSize: '14px', fontFamily: 'Arial' }}>← Tilbage</button>
